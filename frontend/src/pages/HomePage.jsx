    ...
    <div className="page-wrap">
      ...
      <main className="home-main">
        ...
        <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", justifyContent: "center"}}>
          <button className="outline-btn ">Forum Search</button>
          <button className="primary-btn btn-inline" onClick={() => navigate("/login")}>Sign In to Forum</button>
        </div>
      </main>
      ...
      <footer className="footer-nav">
        {["About", "Help", "Privacy", "Terms"].map(l => (
          <span key={l} className="link footer-text" style={{ color: "#5f6368" }}>{l}</span>
        ))}
        <span className="link footer-text" style={{ color: "#5f6368" }} onClick={() => navigate("/login")}>Sign in</span>
        <span className="link footer-text" style={{ color: "#5f6368" }} onClick={() => navigate("/register")}>Register</span>
      </footer>
    </div>
  );
}